...
### 2026-08-31: 3F Scrollの段階的releaseと慣性

- Scroll成立中の最初の3→2または3→1で移動履歴とフレーム時刻を保存する。
- 残指は引き続き3Fが所有し、全指離脱まで慣性も別ジェスチャーも開始しない。
- 全指離脱時だけscroll_endedを発行し、保存時刻で既存の慣性gateを評価する。
  慣性ワーカーの開始時刻は実際の全指離脱時刻で、過去へ戻さない。
- 残指を長く保持しても保存候補は維持する。最初の減少時点で既にstaleなら拒否する。
- 残指の本数増加や3F復帰では候補と旧移動履歴を破棄する。再開した移動は新規履歴となる。
  同一本数のまま指が入れ替わった場合はこの本数ベースの処理では識別できない。
- 3→0の直接release、2Fの判定と共通しきい値は従来どおり。
- Action/Tap/Pinchは対象外。リセットと全指離脱時に保存候補を消去する。
- 診断のGISAVEは保存時点、GIREL version=2のtは実際の終了時刻、evalは判定時刻。
  GIGATEのtはevalに一致する。saved=0は従来どおりt=eval。
